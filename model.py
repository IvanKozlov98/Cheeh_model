...
class Model:
    ...
    def init_traces(self):
        ...
        self.specific_immunity_traces = np.zeros(shape=(Model.TRACES_COUNT_A, Model.TRACES_COUNT_B, Model.MAX_TIME_INFECTED))
        for (i, a) in enumerate(np.linspace(-0.9, -0.1, Model.TRACES_COUNT_A)):
            for (j, b) in enumerate(np.linspace(1, 10, Model.TRACES_COUNT_B)):
                specific_immunity_trace = np.array([1 / (1+np.exp(a * x + b)) for x in range(Model.MAX_TIME_INFECTED)])
    ...
    def make_person_infected(self, person_id, viral_load=800):
        ...
        person.a_id = min(round(person.treatment_rate * Model.TRACES_COUNT_A), Model.TRACES_COUNT_A - 1)
        person.b_id = min(round((person.start_treatment_time - 1) * Model.TRACES_COUNT_B / 10.0), Model.TRACES_COUNT_B - 1)
    ...
    def __init__(self,
        ...
        self.DEAD_THRESHOLD = int(get_value_from_config(Model._SECTION_CONFIG, 'DEAD_THRESHOLD'))
    ...
    def infect(self, infected_person, contact_person, interaction):
        ...
        giving_viral_load = interaction.degree * (infected_person.viral_load / 630) * (1 - contact_person.specific_immun)
    ...
    def _get_contact_list_of_person(self, person):
        # print(person.treatment_rate, person.start_treatment_time, person.age)
        # print(self.viral_load_traces[person.a_id, person.b_id])
    ...
    def _update_static_contact_lists(self, dead_ids):
        for dead_id in dead_ids:
            for (person_id, _) in self.people[dead_id].static_contact_list:
                self.people[person_id].static_contact_list = [(x, interaction) for (x, interaction) in self.people[person_id].static_contact_list if x != dead_id]
    ...
    def _update_state_of_infected_people_step(self):
        ...
        for infected_person_id in self.infected_people_ids:
            ...
            infected_person.viral_load = self.viral_load_traces[infected_person.a_id, infected_person.b_id][infected_person.time_in_infected_state]
            infected_person.specific_immun = self.specific_immunity_traces[infected_person.a_id, infected_person.b_id][infected_person.time_in_infected_state]
    ...
    def run(self, debug_mode=True):
        ...
        for num_day in range(self.num_days):
            number_new_infected_people, number_recovered_people, number_all_infected_people, number_dead_people = self.update(num_day, debug_mode)
