...
def run_cmdline_version(virus_config, model_config, city_config):
    model = Model(
...
def main():
    args = parse_cmdline()
    np.random.seed(42)
    if (args.virus_config is not None) or (args.virus_config is not None) or (args.virus_config is not None):
